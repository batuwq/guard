...
if (message.author.id === uye) {
    ...
    const embed = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor({ name: message.member.displayName, iconURL: author.avatarURL({ dynamic: true, size: 2048 }) }).setTimestamp().setFooter((config.bot.BotFooter), message.guild.iconURL()) 
